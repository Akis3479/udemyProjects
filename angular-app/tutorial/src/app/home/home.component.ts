...
export class HomeComponent {
  ...
  fetchProducts(page: number, perPage: number) {
    this.productsService.getProducts('http://localhost:3000/clothes', { page, perPage: perPage })
    .subscribe({
      next: (data: Products) => {
  ...
  addProduct(product: Product) {
    this.productsService.addProduts('http://localhost:3000/clothes', product).subscribe(
      {
        next: (data) => {
          console.log(data);
          this.fetchProducts(0, this.rows);
